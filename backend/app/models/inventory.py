...
from sqlalchemy import Column, Integer, String, Text, DateTime, ForeignKey, DECIMAL, Enum, Index, Float
...
class Vendedor(Base):
    ...
    ajustes = relationship("AjusteInventarioVendedor", back_populates="vendedor", cascade="all, delete-orphan")
...
class Producto(Base):
    ...
    ajustes = relationship("AjusteInventarioVendedor", back_populates="producto", cascade="all, delete-orphan")
...
class VentaVendedor(Base):
    ...
    notas = Column(Text)
    creado_por = Column(Integer)  # ID del admin que registró la venta
...
class AsignacionProductoVendedor(Base):
    ...
    asignado_por = Column(Integer)  # ID del admin que hizo la asignación
    notas = Column(Text)
...
class AjusteInventarioVendedor(Base):
    __tablename__ = "ajustes_inventario_vendedor"
    
    id = Column(Integer, primary_key=True, autoincrement=True)
    vendedor_id = Column(Integer, ForeignKey("vendedores.id", ondelete="CASCADE"), nullable=False, index=True)
    producto_id = Column(Integer, ForeignKey("productos.id", ondelete="CASCADE"), nullable=False)
    tipo_ajuste = Column(Enum('aumento', 'disminucion'), nullable=False)
    cantidad = Column(Integer, nullable=False)
    cantidad_anterior = Column(Integer, nullable=False)
    cantidad_nueva = Column(Integer, nullable=False)
    razon = Column(Text)
    ajustado_por = Column(Integer)  # ID del admin que hizo el ajuste
    fecha_ajuste = Column(DateTime, default=datetime.utcnow)
    
    # Relaciones
    vendedor = relationship("Vendedor", back_populates="ajustes")
    ...
    def to_dict(self):
        return {
            ...
            "razon": self.razon,
            "ajustado_por": self.ajustado_por,
            "fecha_ajuste": self.fecha_ajuste.isoformat() if self.fecha_ajuste else None
        }